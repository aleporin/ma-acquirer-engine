...
Financial and classification semantics apply to every field, including reasoning

- deal_size_mm is whole-enterprise EV. ev_ebitda_multiple and ev_revenue_multiple
  are enterprise-value multiples for EVERY row, including Minority Investment.
  Never reinterpret them as the amount paid for a stake or say they are not
  whole-company values. Structure can limit direct comparison, but the dataset
  does not separately establish control premiums, equity contribution, exchange
  ratios or adjustments for contingent consideration. Do not invent those terms.
- Use "transaction size" or "enterprise value", never "check size" or "equity
  check". Do not write target-relative size/margin comparisons, "same band",
  "brackets", "nearest", "closest", "largest" or "smallest" in any field.
  The tables expose actual financials; use the prose to explain sector evidence,
  recorded rationale themes and uncertain transaction implications instead.
- Use "closed transactions", never integration experience or integration
  capacity. Closing history does not establish operating competence or outcomes.
- Use the exact sector field. A company with Behavioral Health in its NAME may
  still have sector Healthcare Services; classify it as Healthcare Services.
  Apply this rule to the private reasoning summary as well as public prose.
- Valuation caveats should name the comps' recorded structures and say that
  transaction terms limit direct read-across. Do not invent a platform's identity,
  post-deal ownership, financing terms, premium or consideration allocation.
  Do not compare a comp's geography, financing, stake or ownership to the target.
  The target's sector, EV, margin and prior ownership ARE supplied.
...
- valuation_context: for each selected Closed comp, state its individual
  ...
  in comps. Then explain comparability limits using the comps' own recorded fields.
  State only the recorded structure as a comparability limit; the source does
  not isolate the economic effect of that structure or any control premium. Do not give a range.
...
real row is insufficient if that row does not establish the asserted attribute. Final checks: all multiples remain enterprise-value multiples, regardless of
structure. No target-relative financial comparisons or stake-price interpretations.
No sector inferred from a company name, including in reasoning. No integration
performance inferred from closing. Leave actual financials to the source tables. Return only the structured rationale. Use two short sentences per main section, one per precedent and risk, and one
