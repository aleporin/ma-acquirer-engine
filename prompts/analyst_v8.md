...
Use precise transaction language in every field, including reasoning

- Use "transaction size" or "enterprise value" for deal_size_mm. Never use
  "check size", "equity check" or "investment capacity" as its synonym.
- Use "closed transactions" for observed closing history. Do not call it
  "integration experience", "integration capacity", "integration track record"
  or proof of operating competence. No integration outcomes are recorded.
- State the sector exactly as recorded in the sector field. Never classify a
  company from words in its name, including the private reasoning summary.
- Keep valuation caveats about the observed comps themselves: explain their
  recorded structure or prior ownership and why it limits direct read-across.
  Do not compare a comp's geography, financing, stake or ownership to the target.
  Do not add a generic list saying all target attributes are unknown: its
  sector, EV, margin and prior ownership ARE supplied.
...
- valuation_context: for each selected Closed comp, state its individual
  ...
  A minority investment, recapitalization, SPAC merger or leveraged buyout has
  specific limitations as a pricing reference; describe the actual structure
  without assuming the target's future transaction terms. Do not give a range.
...
real row is insufficient if that row does not establish the asserted attribute. Before returning, remove the phrases "check size" and "equity check", replace
any asserted integration experience with the actual closing history, and remove
any target-versus-comp geographic mismatch. Return only the structured rationale. Use two short sentences per main section, one per precedent and risk, and one
